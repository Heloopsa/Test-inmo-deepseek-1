...
function LoginForm() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (data.user) {
        sessionStorage.setItem('auth-message', '¡Bienvenido de vuelta a InmuebleRD!')
  ...
  return (
    <div className="min-h-screen bg-linear-to-br from-emerald-50 via-white to-teal-50 flex items-center justify-center px-4 py-8">
      <div className="w-full max-w-md">
        ...
        <div className="rounded-2xl border bg-white p-8 shadow-lg">
          ...
          <button
            ...
            disabled={loading}
            className="mb-4 flex w-full items-center justify-center gap-2 rounded-lg border bg-white px-4 py-2.5 text-sm font-medium text-gray-700 shadow-sm transition-all hover:bg-gray-50 disabled:opacity-50"
          >
...
function getErrorMessage(message: string): string {
  const messages: Record<string, string> = {
    ...
    'Email not confirmed': 'Tu email no está confirmado. Revisa tu bandeja de entrada.',
